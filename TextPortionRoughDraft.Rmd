...
The data that we have gathered consists of many cities spread across a majority of states in the US and short term rental tax rates in the US. Our data covers all available data from our data source located in the U.S. This consists of the most recent months data for 32 cities. In our code, we manipulate this data by first importing all of the files in via .csv files we downloaded to our GitHub Repository then importing all of the data into individual data frames with the city name as the data frame title. Next, we add a location column to all of the data frames as there was no prior labeling present in the imported data. This makes the final merged data usable and was a crucial step of our process. After each data frame had mutated a column with their location, we bound the data frames together into our "final" tidy data set and proceeded with the general cleaning process.
